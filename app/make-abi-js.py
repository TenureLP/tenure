"""Generates app/abi.js from the compiled LeaseVault and TestPositionFaucet artifacts.
...
It also carries the topic of every vault event the testnet quests are read from, and the one call
the page makes on the test position faucet.

All of it is derived here from the artifacts forge produced, so the page cannot drift from the
contracts it is talking to. `check.sh` runs this with --check for the same reason it does for
...
FAUCET = os.path.join(ROOT, "lease-vault", "out", "TestPositionFaucet.sol", "TestPositionFaucet.json")
...
# The events the testnet quests are counted from. Each names who did it in an indexed topic, or
# names the deal, whose parties the page already knows from `Listed` and `Funded`.
EVENTS = [
    "Listed", "Cancelled", "Funded", "FeesCollected", "RentClaimed", "BoughtBack", "Released",
    "FinancierTransferred",
]
...
def topic(entry):
    return "0x" + keccak256(signature(entry).encode()).hex()


def build():
    with io.open(ARTIFACT, encoding="utf-8") as fh:
        abi = json.load(fh)["abi"]
    with io.open(FAUCET, encoding="utf-8") as fh:
        faucet_abi = json.load(fh)["abi"]

    fns, errs, structs, events, faucet = {}, {}, {}, {}, {}
    seen = set()

    for entry in faucet_abi:
        if entry.get("type") == "function" and entry["name"] == "give":
            faucet[signature(entry)] = selector(signature(entry))
        if entry.get("type") == "event" and entry["name"] == "Given":
            faucet["Given"] = topic(entry)
    if sorted(faucet) != ["Given", "give()"]:
        raise SystemExit("the faucet artifact has no give() or no Given event")

    for entry in abi:
        kind = entry.get("type")
        if kind == "event" and entry["name"] in EVENTS:
            events[entry["name"]] = topic(entry)
            continue
        if kind == "error":
    ...
    missing = sorted(set(EVENTS) - set(events))
    if missing:
        raise SystemExit("the artifact has no such event: " + ", ".join(missing))

    return fns, errs, structs, events, faucet


def render(fns, errs, structs, events, faucet):
    ...
    ev_lines = "\n".join("    %s: %s," % (q(name), q(t)) for name, t in sorted(events.items()))
    faucet_lines = "\n".join("    %s: %s," % (q(k), q(v)) for k, v in sorted(faucet.items()))
...
if __name__ == "__main__":
    for path in (ARTIFACT, FAUCET):
        if not os.path.exists(path):
            raise SystemExit("no artifact at %s -- run `forge build` in lease-vault first" % path)

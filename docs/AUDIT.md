...
| Medium | Listings stayed fundable for days after the committee revoked a pool or paused listings, and the position could drift out of range between listing and funding. | `fund()` re-reads the allowlist, the pause flag and the in-range test. This also makes spot-tick manipulation uneconomic: it must now hold across two transactions whose timing the attacker does not control. |
| Medium | The freeze probe was guessed. A token answering `paused()` in any other shape was silently read as "not frozen", which is the exact inversion of the design intent. | The registry records per pool how to ask. With a probe declared present, an answer in the wrong shape is read as frozen rather than ignored. |
| Low | `feeRecipient` was read live at funding while the contract claimed every registry input was snapshotted at listing. | Snapshotted into the deal. |
